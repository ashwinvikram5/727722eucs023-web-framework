...
public class AshwinProductController {
    ...
    public ResponseEntity<List<AshwinProduct>> getAll(@PathVariable("name") String name) {
        List<AshwinProduct> li = ser.start(name);
        if (li.size() > 0) {
    ...
    public ResponseEntity<List<AshwinProduct>> getbyAge(@PathVariable("name") String name) {
        List<AshwinProduct> li = ser.end(name);
        if (li.size() > 0) {
            return new ResponseEntity<>(li, HttpStatus.OK);
        } else {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
    ...
    public ResponseEntity<List<AshwinProduct>> getbyge() {
        List<AshwinProduct> li = ser.sort();
        if (li.size() > 0) {
            return new ResponseEntity<>(li, HttpStatus.OK);
        } else {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
